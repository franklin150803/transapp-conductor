...
// viven en utils.js (debe cargarse antes que este archivo).
        ...
        async function showRoute(companyId, company) {
            ...
            const weight = isSelected ? 3.5 : (isRegistered ? 2 : 1.2);
            const opacity = isSelected ? 0.95 : (isRegistered ? 0.55 : 0.25);
            const dash = isSelected ? null : (isRegistered ? null : '10, 8');

            // Colores más limpios para las rutas (verde esmeralda neón para ida, rosa neón para retorno)
            const idaColor = isRegistered ? '#10b981' : '#1b4d3e';
            const retornoColor = isRegistered ? '#f43f5e' : '#7f2d3d';
