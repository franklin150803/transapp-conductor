...
// script principal.
        ...
        function startDriverMode() {
            ...
            setTimeout(() => {
                if (!driverMap) {
                    ...
                    window.driverGlMap = driverGlLayer.getMaplibreMap();
                ...
                // Rutas dibujadas como layers nativos de MapLibre (misma
                // escena 3D que el mapa del pasajero) en vez de L.polyline de
                // Leaflet, para que respeten el pitch/bearing en vez de
                // flotar como un overlay plano encima del mapa inclinado.
                window.waitForGlMapReady(window.driverGlMap, (glMap) => {
                    if (company && company.routePointsIda) {
                        window.upsertRouteLine(glMap, 'driver-ida', company.routePointsIda, {
                            color: '#10b981', weight: 3, opacity: 0.6, dash: [1.3, 1]
                        });
                    }
                    if (company && company.routePointsRetorno) {
                        window.upsertRouteLine(glMap, 'driver-retorno', company.routePointsRetorno, {
                            color: '#f43f5e', weight: 3, opacity: 0.6, dash: [1.3, 1]
                        });
                    }
                });
            }, 50);
        ...
        function shouldSendToFirebaseNow(speedKmh) {
            ...
            const mode = window.getAppSetting ? window.getAppSetting('gpsInterval', '5') : '5';
            let minInterval;
            if (mode === '2') {
                minInterval = 2000;
            } else if (mode === '10') {
        ...
        let lastAdjacentCalculationTime = 0;

        function updateAdjacentVehicles(companyId, vehicleId, lat, lng, ownSpeedKmh) {
            const now = Date.now();
            if (now - lastAdjacentCalculationTime < 15000) {
                return;
            }
            lastAdjacentCalculationTime = now;
        ...
        async function flushOfflineQueue() {
            ...
            try {
                const updates = {};
                // Usamos la última lectura en cola para la ubicación en vivo
                const latestReading = queue[queue.length - 1];
                updates[`vehiculos_live/${companyId}/${vehicleId}`] = latestReading;

                await window.fbUpdate(window.fbRef(window.fbDb), updates);
                
                clearOfflineQueue();
                addLog(`${queue.length} posiciones procesadas (última enviada). Transmisión normal restablecida.`);
            } catch (e) {
                addLog('Error al enviar las posiciones guardadas. Se reintentará luego.');
                console.error(e);
            } finally {
