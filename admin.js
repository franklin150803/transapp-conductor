...
// principal.
        ...
        function registerCompany() {
            ...
            const colors = ['#ec4899', '#3b82f6', '#f97316', '#06b6d4', '#94a3b8', '#8b5cf6'];
        ...
        let emergenciesListenerUnsub = null;

        function listenForEmergencies() {
            if (!window.firebaseReady) return;
            if (emergenciesListenerUnsub) {
                emergenciesListenerUnsub();
                emergenciesListenerUnsub = null;
            }
            const ref = window.fbRef(window.fbDb, 'emergencias');
            emergenciesListenerUnsub = window.fbOnValue(ref, (snap) => {
                const data = snap.val() || {};
                const active = [];
        ...
        function loadAndRenderHistory() {
            ...
            const historyQuery = window.fbRef(window.fbDb, 'historial').limitToLast(100);
            window.fbGet(historyQuery).then(snap => {
