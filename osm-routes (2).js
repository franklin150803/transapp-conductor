// osm-routes.js v2 — espera window.map expuesto desde passenger.js
...
(function () {
        ...
        try {
            ...
            lineas.forEach(function (coords) {
                const pl = L.polyline(coords, {
                    color: COLOR_OSM, weight: WEIGHT_OSM,
                    opacity: OPACITY_OSM, smoothFactor: 1.5,
                    interactive: false, className: 'osm-route-layer'
                }).addTo(lmap);
                pl.bringToBack();
                osmLayers.push(pl);
            });
    ...
    // Esperar a que window.map esté disponible (lo expone passenger.js)
    let intentos = 0;
    const intervalo = setInterval(function () {
        ...
        } else if (intentos > 200) { // 20 segundos máximo
            clearInterval(intervalo);
