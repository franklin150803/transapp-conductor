...
// cargarse antes que este archivo (usa window.fbAuth, window.fbSignIn, etc).
        ...
        function applyAuthUI(user, profile) {
            ...
            // Un pasajero ve Pasajero y Perfil. Un conductor ve Conductor y Perfil. El Admin ve todos.
            ...
            if (nameVal) nameVal.textContent = (profile && profile.name) || (window.fbAuth.currentUser ? window.fbAuth.currentUser.displayName : '') || 'Invitado';
            if (emailVal) emailVal.textContent = (window.fbAuth.currentUser ? window.fbAuth.currentUser.email : '') || 'No registrado';
            if (roleVal) roleVal.textContent = isAdmin ? 'Administrador' : (userRole === 'conductor' ? 'Conductor' : 'Pasajero');
